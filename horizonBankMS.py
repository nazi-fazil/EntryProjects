...
class BankManagementSystem:
    ...
    def deposit(self, username):
        label_amount = tk.Label(self.operations_frame, text="Amount(Rs)")
        label_amount.pack(pady=5)
        entry_amount = tk.Entry(self.operations_frame)
        entry_amount.pack(pady=5)
        submit_button = tk.Button(self.operations_frame,text="Submit",command=lambda: self.submit_deposit(username,entry_amount))
    ...
    def submit_deposit(self,username,entry_amount):
        ...
        try:
            ...
            with open(file_name, "w") as file1:
                json.dump(data, file1)
                messagebox.showinfo("Balance",f"Your Account balance is {data['balance']}")
        except Exception as e:
    ...
    def withdraw(self, username):
        ...
        entry_amount = tk.Entry(self.operations_frame)
        entry_amount.pack(pady=5)
        submit_button = tk.Button(self.operations_frame, text="Submit",command=lambda: self.submit_withdraw(username, entry_amount))
        submit_button.pack(pady=5)
